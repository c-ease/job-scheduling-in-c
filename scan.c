...
void merge(int n, int dir, int first[n], int mid, int second[n], int end, int sorted[n+2])
{
	int i, c = 0;
	for(i=0; i<n; i++)
	{
...
int sort(int n, int head, int dir, int max, int string[n], int left[n], int right[n], int sorted[n+2])
{
	int i, a = 0, b = 0;
	for(i=0; i<n; i++)
	{
	...
	desc(n,left); asc(n,right); //display(n,left); display(n,right);
	switch(dir)
	{
		case 0: merge(n,dir,left,0,right,max,sorted); break;
		case 1: merge(n,dir,right,max,left,0,sorted); break;
	}
	return a;
...
int main()
{
	...
	scanf("%d",&n);
	int string[n], left[n], right[n], sorted[n+2];
	...
	total = displayList(head,n+2,sorted,dir,total,max);
